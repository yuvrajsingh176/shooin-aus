import { FaInstagram } from "react-icons/fa6";
import { FaFacebook, FaYoutube, FaTiktok } from "react-icons/fa";
import { useState } from "react";

const Navbar = () => {
  const [close, setClose] = useState(false);

  return (
    <div className={`text-[#FFC700]     md:w-[95%] w-full h-full md:my-6`}>
      <div
        className="flex md:hidden   justify-between mr-10 mt-4"
        onClick={() => {
          setClose(!close);
        }}
      >
        <img
          className="h-[50px] ml-10 md:hidden rounded-lg  w-[50px]"
          src="data:image/jpeg;base64,"
        ></img>
        <div className="md:hidden">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
            stroke="currentColor"
            className="w-10  h-10"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"
            />
          </svg>
        </div>
      </div>
      <div
        className={`flex fixed z-50 w-full md:top-6 top-0 right-0 flex-col md:h-auto h-screen ${
          close && "hidden"
        } bg-gray-400 md:bg-transparent md:flex-row text-xl font-semibold cursor-pointer transition ease-in md:justify-around items-center text-center`}
      >
        <img
          className="h-[50px] hidden md:block rounded-lg  w-[50px]"
          src="data:image/jpeg;base64,"
        ></img>
        <p
          onClick={() => {
            setClose(!close);
          }}
          className="hover:text-[#EDF5E1] md:hidden  flex justify-end mr-20 mt-4 w-full text-black text-4xl transition ease-in my-6 md:my-0"
        >
          X
        </p>
        <a href="#home">
          <p className="hover:text-[#EDF5E1] transition ease-in my-6 md:my-0">
            Home
          </p>
        </a>
        <a href="#giveaway">
          <p className="hover:text-[#EDF5E1] transition ease-in my-6 md:my-0">
            Giveaways
          </p>
        </a>
        <a href="#pastwin">
          <p className="hover:text-[#EDF5E1] transition ease-in my-6 md:my-0">
            Past Winners
          </p>
        </a>
        <a href="#winweek">
          <p className="hover:text-[#EDF5E1] transition ease-in my-6 md:my-0">
            Discounts
          </p>
        </a>
        <a href="#plan">
          <p className="hover:text-[#EDF5E1] transition ease-in my-6 md:my-0">
            Membership
          </p>
        </a>
        <a href="#about">
          <p className="hover:text-[#EDF5E1] transition ease-in my-6 md:my-0">
            About Us
          </p>
        </a>

        <div className="md:flex hidden  flex-col md:flex-row">
          <FaInstagram className="md:mr-2 hover:text-[#EDF5E1] mr-8 text-4xl transition ease-in" />
          <FaFacebook className="md:mr-2 mr-8 hover:text-[#EDF5E1] text-4xl transition ease-in" />
          <FaYoutube className="md:mr-2 mr-8 hover:text-[#EDF5E1] text-4xl transition ease-in" />
          <FaTiktok className=" hover:text-[#EDF5E1] text-4xl transition ease-in" />
        </div>
      </div>
    </div>
  );
};
export default Navbar;
